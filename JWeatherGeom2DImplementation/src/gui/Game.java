...
public class Game extends JPanel {
	...
	static double initialX, initialY, distanceX, distanceY;
	...
	Game() {
					...
					if (MANUAL) {
						...
						if (!menu.baseRect.contains(new Point(e.getX(), e.getY()))) {
							if (Game.gravityMode != 0) {
								particleList.add(new Particle(new Point2D(initialX, initialY), 8, new Vector2D(distanceX / 40, distanceY / 40), 5, 1f, 0));
							} else {
								particleList.add(new Particle(new Point2D(initialX, initialY), 8, new Vector2D(distanceX / 20, distanceY / 20), 5, 1f, 0));
							}
						}
					}
				} else if (e.getButton() == MouseEvent.BUTTON3) {
		...
		orbitalCircle = new Circle2D(new Point2D(cImageD.getWidth() / 2, cImageD.getHeight() / 2), cImageD.getHeight() / 2);
	}
	...
	public void update() {
		...
		for (int i = 0; i < particleList.size(); i++) {
			try {
				...
				if (currentParticle.TYPE == 0) {
					if (currentParticle.center().getX() > width || currentParticle.center().getX() < 0) {
						particleList.remove(i);
					} else {
						currentParticle.update();
					}
				} else if (currentParticle.TYPE == 1) {
					if (currentParticle.center().getX() > (width + offScreenCalculationSnow()) || currentParticle.center().getX() < -offScreenCalculationSnow() || currentParticle.age > currentParticle.lifetime) {
						particleList.remove(i);
					} else {
						currentParticle.update();
					}
				} else if (currentParticle.TYPE == 2) {
					if (currentParticle.center().getX() > (width + offScreenCalculationRain()) || currentParticle.center().getX() < 0 - offScreenCalculationRain() || currentParticle.center().getY() > height) {
						particleList.remove(i);
					} else {
						currentParticle.update();
					}

					if (currentParticle.center().getY() > height - 15 && currentParticle.center().getY() < height) {
						for (int x = 0; x <= 3; x++) {
							double calculatedX = 0;
							if (randInt(0, 2) == 1) {
								calculatedX = -((currentParticle.velocity.getX() / 2) + randDouble(1, -1));
							} else {
								calculatedX = ((currentParticle.velocity.getX() / 2) + randDouble(1, -1));
							}
							particleList.add(new Particle(new Point2D(currentParticle.center().getX(), currentParticle.center().getY()), 2, new Vector2D(calculatedX, randDouble(0, -2.0)), 1, currentParticle.alpha, 3));
						}
					}
				} else if (currentParticle.TYPE == 3) {
					if (currentParticle.center().getX() > width || currentParticle.center().getX() < 0 || currentParticle.age > currentParticle.lifetime) {
						particleList.remove(i);
					} else {
						currentParticle.update();
					}
		...
		if (frames % TARGET_FPS == 0) {
			...
			snowTimer.schedule(new TimerTask() {
				...
				public void run() {
					if (SNOW) {
						particleList.add(new Particle(new Point2D(xSpawnPosition(1), -2), randInt(2, 5), new Vector2D(snowX, snowY), 10, (float) randDouble(0.01, 1), 1));
						snowCount++;
					}
				}
			}, 0, snowDelay);
			...
			rainTimer.schedule(new TimerTask() {
				...
				public void run() {
					if (RAIN) {
						particleList.add(new Particle(new Point2D(xSpawnPosition(2), -2), randInt(2, 3), new Vector2D(rainX, rainY), 5, (float) randDouble(0.01, 0.8), 2));
					}
				}
			}, 0, rainDelay);
			...
			spawnTimer.schedule(new TimerTask() {
				...
				public void run() {
					if (rightClicked && MANUAL) {
						particleList.add(new Particle(new Point2D(mouseX, mouseY), 8, new Vector2D(0, 0), 5, 1f, 0));
					}
				}
			}, 0, spawnDelay);
		}
	...
	public void paint(Graphics g) {
		...
		try {
			...
			for (int i = 0; i < particleList.size(); i++) {
				if (gravityMode != 2 || (gravityMode == 2 && particleList.get(i).velocity.getX() > 0)) {
			...
			if (gravityMode == 2) {
				...
				g2d.draw(orbitalCircle.asAwtShape());
			}

			for (int i = 0; i < particleList.size(); i++) {
				if (gravityMode == 2 && particleList.get(i).velocity.getX() <= 0) {
					particleList.get(i).paint(g2d);
				}
			}
		...
		if (dragging) {
			Line2D line = new Line2D(initialX, initialY, mouseX, mouseY);
			g2d.draw(line.asAwtShape());
		}
